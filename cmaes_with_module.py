...
#print(cma.CMAOptions('verb')) # display verbosity options





def cmaes_res(G,M,N,tol=1e-1):
    ...
    res = cma.fmin2(objective_fct,x0, std0, {'integer_variables': list(range(M)),'bounds': [0,N+1-1e-9],'tolx':tol } )[0]
    return res
...
G2 = rx.networkx_converter(G)
G3 = G2.copy()
G3.remove_nodes_from(res.astype(int))
score = max_ev(G3)
